...
void mat_mul( float *c, float *a, float *b, int NDIM ) {
	...
	for (i = 0; i < NDIM; i++) {
		for (j = 0; j < NDIM; j++) {
			a_elm = *(a + (i * NDIM + j));
			float a_tmp[4] = {a_elm, a_elm, a_elm, a_elm};
			v4sf A = __builtin_ia32_loadups(a_tmp);
			for (k = 0; k + 3 < NDIM; k += 4) {
				v4sf B = __builtin_ia32_loadups(b + (j * NDIM + k));
				v4sf M = __builtin_ia32_mulps(A, B);
				v4sf C = __builtin_ia32_loadups(c + (i * NDIM + k));
				v4sf R = __builtin_ia32_addps(C, M);
				__builtin_ia32_storeups(c + (i * NDIM + k), R);
			}
			for (; k < NDIM; k++) {
