...
const char* kernel_src = 
				...
				"	float result = 0;" \
				"	for (i = 0; i < NDIM; i++) {" \
				"		result += A[id1 * NDIM + i] * B[id2 * NDIM + i];" \
				"	}" \
...
void mat_mul( float *c, float *a, float *b, int NDIM, int NGPU, int NCPU) {
	...
	// transpose
	float *b_t = (float *)malloc(NDIM * NDIM * sizeof(float));
	for (i = 0; i < NDIM; i++) {
		for (j = 0; j < NDIM; j++) {
			b_t[j*NDIM + i] = b[i*NDIM + j];
		}
	}
	...
	for (i = 0; i < device_num; i++) {
		...
		error = clEnqueueWriteBuffer(command_queue[i], bufferB[i], CL_FALSE, 0, sizeB, b_t, 0, NULL, NULL);
		if (error != CL_SUCCESS) {
