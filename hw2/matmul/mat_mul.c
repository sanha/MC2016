...
void *Calc(void* arg) {
	...
	int *offset = (int *)arg;
	// C = AB
	for (i = *offset; i < *offset + NDIM / NUM_THREADS; i++) {
		for (j = 0; j < NDIM; j++) {
			for (k = 0; k < NDIM; k++) {
				c[i][k] += a[i][j] * b[j][k];
			}
...
void mat_mul( float c[NDIM][NDIM], float a[NDIM][NDIM], float b[NDIM][NDIM] )
{
	...
	for (i = 0; i < NUM_THREADS; i++) {
		param[i] = offset;
		offset += NDIM / NUM_THREADS;
		pthread_create(&threads[i], NULL, Calc, (void *)&param[i]);
	}

	void *status;
	pthread_join(threads[NUM_THREADS - 1], &status);
}
...
void check_mat_mul( float c[NDIM][NDIM], float a[NDIM][NDIM], float b[NDIM][NDIM] )
{
	...
	for( i = 0; i < NDIM; i++ )
	{
		for( j = 0; j < NDIM; j++ )
		{
			...
			if( (c[i][j] - sum)/sum < -eps || (c[i][j] - sum)/sum > eps)
			{
